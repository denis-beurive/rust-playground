...
fn moving_test() {
    // Moving a variable means that the variable has been assigned to a new owner.
    // - Rust moves by default. This means that it copies the bytes to the new location,
    //   and the old object is then gone.
    // - Assigning a value to another variable moves it.

    let var1: String = String::from("test string");
    println!("var1: \"{}\" ({:p})", var1, &var1);

    // Let's move the value of "var1" to "var2".
    // - The string is copied from the location pointed by "var1" to the location
    //   pointed by "var2".
    // - And then, the location designated by "var1" is freed.
